...
if(isset($_SESSION['items_in_sale']))
{
	...
	{
	echo "<td colspan=7><center><h3>$lang->restaurantOrder $lang->orderFor: <b>$order_customer_name</b></h3></td>
...
{
	 // For first screen only, display radio buttons to select restaurant or takeaway
...
{
  	$item_result=mysql_query("SELECT item_name,unit_price,tax_percent,brand_id,id,takeawayprice FROM $items_table ORDER by item_name LIMIT 0,200",$dbf->conn);
}
elseif(isset($action))
{
...
{
 	$item_result=mysql_query("SELECT item_name,unit_price,tax_percent,brand_id,id,takeawayprice FROM $items_table ORDER by item_name",$dbf->conn);
}
...
if(isset($_SESSION['items_in_sale']))
{
	...
	<td rowspan=1><font color=CCCCCC><b>$lang->tax %</b></font></td>
...
while($row=mysql_fetch_assoc($brand_result))
{
	...
	echo "<li><a href='sale_ui.php?action=$bid'><font color='0000FF'><b>$display_brand</b></font></a></li>\n";
...
if(isset($_SESSION['items_in_sale']))
{
	...
	for($k=0;$k<$num_items;$k++)
	{
		...
		$temp_tax=$item_info[2]+100;
		$temp_quantity=$item_info[3];
		echo "temp_price is $temp_price , temp_tax is $temp_tax";
		$rowTotal=$temp_price*$temp_quantity;

		$tax=$rowTotal-(($rowTotal/$temp_tax)*100);
