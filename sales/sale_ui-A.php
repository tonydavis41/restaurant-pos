...
$lang=new language();
//Debug - check if anything in $_GET
if(empty($_GET)) {
...
//item_id=$item_info[0];
//cho "DEBUG: quantity is $new_quantity for $item_id";
///$_SESSION['items_in_sale'][$k]=$item_id.' '.$new_price.' '.$new_tax.' '.$new_quantity;
//_SESSION['items_in_sale'][$k]=$item_id.' '.$new_quantity;
//eader("location: sale_ui.php");

//
//se
//
//  echo "DEBUG: update item not set k is $k";
//
...
//Initiate the asyncronous request - activated when hyperlink in box 1 clicked
//function updatePrice(theupdate,theprice) {
//	var newquantity = theupdate;
//	alert("You entered function updatePrice" +newquantity);
//	if(isEmpty(newquantity.value)) {
//		newquantity.value = '1';
//	} else if(isInteger(newquantity.value)){
//		theprice = theprice * newquantity.value;
//	} else {
...
//	if (receiveReq.readyState == 4 || receiveReq.readyState == 0) {
		//Setup the connection as a GET call to test.html.
		//True explicity sets the request to asyncronous (default).
//		receiveReq.open("GET", 'update_price.php?total='+thetotal'?quantity='+newquantity, true);
		//Set the function that will be called when the XmlHttpRequest objects state changes.
//		receiveReq.onreadystatechange = setPrice; 
		//Make the actual request.
//		receiveReq.send(null);
//	}			
//}
...
//	if (receiveReq.readyState == 4) {
		//Set the contents of box 2 (id='items') to test.html.
//		document.getElementById('theprice').innerHTML = receiveReq.responseText;
//	}
//}
// Form validation
...
//{
//  $thequantity=$_SESSION['quantity'];
//  echo "quantity is set to $thequantity";
 // if(isset($_POST['items']))
 // {
//	$theitem=$_POST['items'];
//	echo "the item is $theitem";
// }
//  
//}
//else
...
echo "<center><form name='additem' method='POST' onsubmit='return validateForm()' action='sale_ui.php'>
...
if(isset($_SESSION['items_in_sale']))
{
	...
	for($k=0;$k<$num_items;$k++)
	{
		...
		//<td id='theprice'><font color='white'><b>$cfg_currency_symbol$rowTotal</b></font></td>
		//<td><input type=text name='quantity$k' value='$temp_quantity' size='3'/></td>";
		//echo "<td><input type='button' value='update' onClick='updatePrice(this.form.elements[\"quantity$k\"],$rowTotal);'/></td></tr>";
		//<td><a href='javascript:updatePrice($rowTotal,$temp_quantity)'><font color='0000FF'><b>Update</b></font></a>
		//<td><input type='button' name='a name' value='fred' onclick='customerPopUp()'></td>
		//<input type='hidden' name='item_id$k' value='$temp_item_id'></tr>";
		echo "<td><font color='white'><b>$temp_item_name ! $k</b></font></td>
